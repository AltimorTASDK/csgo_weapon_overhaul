...
const void *get_vfunc(const void *object, const int idx);
...
public:
	...
	void init(void *hook_target, void *user_data);
	 ...
	 */
	template<typename T> void hook(const int idx, T hook_func)
	{
		((T*)(new_vtable))[idx + 3] = hook_func;
	}
	 ...
	 */
	template<typename func_t> func_t get_orig(const int idx)
	{
